...
function implicit_integrator_solve(params::NamedTuple, dynamics::Function, x1::Vector, dt::Real;tol = 1e-13, max_iters = 10)::Vector{Float64}
    # initialize guess
    x2 = 1*x1
    residual_function = x2 -> implicit_midpoint(params, dynamics, x1, x2, dt)
    x2 = newtons_method(x2, residual_function; max_iters, tol)
    return x2
end    

function simulate_implicit_midpoint(params::NamedTuple,dynamics::Function,x0::Vector,dt::Real,tf::Real; tol = 1e-13)
    t_vec = 0:dt:tf
    N = length(t_vec)
