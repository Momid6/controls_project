...
function kkt_conditions(z::Vector, decision_vars::Int64, cost_fx::Function, constraint_fx::Function)::Vector
    ...
    ∇c = constraint_jacobian(x, constraint_fx)
    c_val = constraint_fx(x)
    return vcat(∇f + ∇c' * λ, c_val)
end


function gn_kkt_jac(z::Vector, decision_vars::Int64, constraint_vars::Int64, cost_fx::Function, constraint_fx::Function)::Matrix
    n = decision_vars
    m = constraint_vars
    x = z[1:n]
    λ = z[n+1:end]
    J = [FD.hessian(cost_fx, x) constraint_jacobian(x, constraint_fx)'; constraint_jacobian(x, constraint_fx) -1e-3*ones(m,m)]
    return J
end


function gauss_newton(z0::Vector, cost_fx::Function, decision_vars::Int64, constraint_fx::Function, constraint_vars::Int64, merit_fx::Function)
    kkt(_z) = kkt_conditions(_z, decision_vars, cost_fx, constraint_fx)
    kkt_jac(_z) = gn_kkt_jac(_z, decision_vars, constraint_vars, cost_fx, constraint_fx)
    Z = newtons_method(z0, kkt, kkt_jac, merit_fx; tol = 1e-4, max_iters = 100, verbose = true)
    return Z
end
